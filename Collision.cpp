...
bool Collision::checkCollision( std::vector<Tile>& tile_vector,  Hero *hero, int x, int y ) {
    ...
    for(const auto& i: tile_vector){
        ...
        if(i.t!=0 && position.x + HERO_SIZE*HERO_SCALE > i.hitLeft && position.x + hero->getGlobalBounds().width/4 < i.hitRight &&
           position.y + hero->getGlobalBounds().height/2 < i.hitBottom && position.y + HERO_SIZE*HERO_SCALE  > i.hitTop){
...
bool Collision::checkCollision(std::vector<Tile> &tile_vector, Enemy *enemy, int x, int y) {
    ...
    for(const auto& i: tile_vector){
        ...
        if(i.t!=0 && position.x + ENEMY_SIZE * ENEMY_SCALE  > i.hitLeft && position.x + enemy->getGlobalBounds().width/6  < i.hitRight &&
           position.y + enemy->getGlobalBounds().height/2 < i.hitBottom && position.y + ENEMY_SIZE*ENEMY_SCALE > i.hitTop){
            //control on the tile, I left side, II right side, III bottom side, IV top side
...
bool Collision::checkCollision(std::vector<Tile> &tile_vector, Boss *boss, int x, int y) {
    ...
    for(const auto& i: tile_vector){
        if(i.t!=0 && position.x + BOSS_SIZE*BOSS_SCALE > i.hitLeft && position.x + boss->getGlobalBounds().width/6  < i.hitRight &&
           position.y + boss->getGlobalBounds().height/2 < i.hitBottom && position.y + BOSS_SIZE*BOSS_SCALE > i.hitTop){
            //control on the tile, I left side, II right side, III bottom side, IV top side
...
bool Collision::projectileCollisionBoss(Projectile *projectile, const Boss &boss, int x, int y) {
    ...
    if(position.x + PROJECTILE_SIZE > boss.getPosition().x + boss.getGlobalBounds().width/3 && position.x < boss.getPosition().x + BOSS_SIZE*BOSS_SCALE &&
